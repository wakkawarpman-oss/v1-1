...
const BUILDER_DEFAULTS: PackBuilderState = {
  s: 6, p: 1, cellId: 'molicel-p42a', loadCurrentA: 30,
...
function PackBuilderCard({ onPackBuilt }: { onPackBuilt: (capacityAh: number, voltageV: number) => void }) {
  ...
  const selectedCell = CELL_DATABASE.find((c) => c.id === state.cellId)
  ...
  return (
    <ToolCard
      ...
      description="S×P конфігурація, падіння напруги під навантаженням (Закон Ома) та тепловтрати (Джоуль–Ленц)."
    >
      <Field label="Комірка">
        <Select title="Комірка" aria-label="Комірка" value={state.cellId} onChange={(e) => setState((s) => ({ ...s, cellId: e.target.value }))}>
          {CELL_DATABASE.map((cell) => (
      ...
      {result && (
        <div className="space-y-1.5">
          ...
          <ResultBox copyValue={formatToolNumber(result.capacityAh, 3)}>
            Ємність: <span className="font-semibold text-ecalc-navy">{formatToolNumber(result.capacityAh * 1000, 0)} мАч</span>
          </ResultBox>
    ...
    <Card className="calc-surface overflow-hidden">
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2.5 text-[15px]">
          ...
          <span className="leading-snug font-semibold text-ecalc-navy">Довідник комірок</span>
        </CardTitle>
        <CardDescription className="text-[11px] leading-relaxed">
          Параметри з даташитів виробників при кімнатній температурі.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="overflow-x-auto">
          <table className="w-full text-xs border-collapse">
            ...
            <tbody>
              {CELL_DATABASE.filter((c) => c.id !== 'custom').map((cell) => (
    ...
    <section className="space-y-6">
      <Card className="calc-surface overflow-hidden">
        <CardHeader>
          ...
          <CardDescription>
            Конструктор акумуляторних збірок із реальними параметрами клітин 21700.
            Розраховує падіння напруги за законом Ома та тепловтрати Джоуль–Ленца.
